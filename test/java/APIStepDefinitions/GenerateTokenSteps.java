...
public class GenerateTokenSteps {
    ...
    public void a_jwt_is_generated() {
        RequestSpecification generateTokenRequest= given().header("Content-Type", "application/json")
                .body("{\n" +
                        "\"email\": \"batch15QA@tester.com\",\n" +
                        "\"password\": \"test123\"\n" +
                        "}");
        Response response=generateTokenRequest.when().post("/generateToken.php");

        //storing the token in global variable
        token= "Bearer "+ response.jsonPath().getString("token"); // token is from postman response
        System.out.println(token); //now our token will generate automatically
    }
}
